...
const __crawdog_curved25519_key __crawdog_curved25519_expected = {
...
/* from __crawdog_ed25519-donna-batchverify.h */
...
__crawdog_ed25519-donna-batchverify.h has been modified to match the 
...
test_batch_instance(batch_test type, uint64_t *ticks) {
	__crawdog_ed25519_secret_key sks[test_batch_count];
	__crawdog_ed25519_public_key pks[test_batch_count];
	__crawdog_ed25519_signature sigs[test_batch_count];
	...
	for (i = 0; i < test_batch_count; i++) {
		__crawdog_ed25519_randombytes_unsafe(sks[i], sizeof(sks[i]));
		__crawdog_ed25519_publickey(sks[i], pks[i]);
		pk_pointers[i] = pks[i];
	}

	/* generate messages */
	__crawdog_ed25519_randombytes_unsafe(messages, sizeof(messages));
	for (i = 0; i < test_batch_count; i++) {
	...
	/* sign messages */
	for (i = 0; i < test_batch_count; i++) {
		__crawdog_ed25519_sign(message_pointers[i], message_lengths[i], sks[i], pks[i], sigs[i]);
	...
	/* batch verify */
	t = get_ticks();
	ret = __crawdog_ed25519_sign_open_batch(message_pointers, message_lengths, pk_pointers, sig_pointers, test_batch_count, valid);
	*ticks = get_ticks() - t;
...
test_main(void) {
	int i, res;
	__crawdog_ed25519_public_key pk;
	__crawdog_ed25519_signature sig;
	unsigned char forge[1024] = {'x'};
	__crawdog_curved25519_key csk[2] = {{255}};
	uint64_t ticks, pkticks = maxticks, signticks = maxticks, openticks = maxticks, curvedticks = maxticks;

	for (i = 0; i < 1024; i++) {
		__crawdog_ed25519_publickey(dataset[i].sk, pk);
		edassert_equal_round(dataset[i].pk, pk, sizeof(pk), i, "public key didn't match");
		__crawdog_ed25519_sign((unsigned char *)dataset[i].m, i, dataset[i].sk, pk, sig);
		edassert_equal_round(dataset[i].sig, sig, sizeof(sig), i, "signature didn't match");
		edassert(!__crawdog_ed25519_sign_open((unsigned char *)dataset[i].m, i, pk, sig), i, "failed to open message");

		memcpy(forge, dataset[i].m, i);
		if (i)
			forge[i - 1] += 1;

		edassert(__crawdog_ed25519_sign_open(forge, (i) ? i : 1, pk, sig), i, "opened forged message");
	}

	for (i = 0; i < 1024; i++)
		__crawdog_curved25519_scalarmult_basepoint(csk[(i & 1) ^ 1], csk[i & 1]);
	edassert_equal(__crawdog_curved25519_expected, csk[0], sizeof(__crawdog_curved25519_key), "curve25519 failed to generate correct value");

	for (i = 0; i < 2048; i++) {
		timeit(__crawdog_ed25519_publickey(dataset[0].sk, pk), pkticks);
		edassert_equal_round(dataset[0].pk, pk, sizeof(pk), i, "public key didn't match");
		timeit(__crawdog_ed25519_sign((unsigned char *)dataset[0].m, 0, dataset[0].sk, pk, sig), signticks);
		edassert_equal_round(dataset[0].sig, sig, sizeof(sig), i, "signature didn't match");
		timeit(res = __crawdog_ed25519_sign_open((unsigned char *)dataset[0].m, 0, pk, sig), openticks);
		edassert(!res, 0, "failed to open message");
		timeit(__crawdog_curved25519_scalarmult_basepoint(csk[1], csk[0]), curvedticks);
	}
