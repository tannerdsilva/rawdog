...
test_adds() {
	...
	__crawdog_curve25519_add(a, max_bignum, max_bignum);

	/* b = ((max_bignum + max_bignum) * (max_bignum + max_bignum)) */
	__crawdog_curve25519_mul(b, a, a);
	__crawdog_curve25519_contract(result, b);
	if (memcmp(result, max_bignum2_squared_raw, 32) != 0)
		return -1;
	__crawdog_curve25519_square(b, a);
	__crawdog_curve25519_contract(result, b);
	if (memcmp(result, max_bignum2_squared_raw, 32) != 0)
		return -1;

	/* b = (max_bignum + max_bignum + max_bignum) */
	__crawdog_curve25519_add_after_basic(b, a, max_bignum);

	/* a = ((max_bignum + max_bignum + max_bignum) * (max_bignum + max_bignum + max_bignum)) */
	__crawdog_curve25519_mul(a, b, b);
	__crawdog_curve25519_contract(result, a);
	if (memcmp(result, max_bignum3_squared_raw, 32) != 0)
		return -1;
	__crawdog_curve25519_square(a, b);
	__crawdog_curve25519_contract(result, a);
	if (memcmp(result, max_bignum3_squared_raw, 32) != 0)
		return -1;
...
test_subs() {
	...
	__crawdog_curve25519_sub(a, max_bignum, zero);
	__crawdog_curve25519_contract(result, a);
	...
	/* b = ((a - 0) - 0) */
	__crawdog_curve25519_sub_after_basic(b, a, zero);
	__crawdog_curve25519_contract(result, b);
	if (memcmp(result, max_bignum_raw, 32) != 0)
		return -1;
