...
extension rawdog_tests {
	...
	struct RAWHMACTests {
		...
		func testVector1() throws {
			...
			try hmac5.finish().RAW_access_immutable(UnsafeRawBufferPointer.self) {
				#expect([UInt8]($0) == expected5)
			}
			try hmac1.finish().RAW_access_immutable(UnsafeRawBufferPointer.self) {
				#expect([UInt8]($0) == expected1)
			}
			try hmac256.finish().RAW_access_immutable(UnsafeRawBufferPointer.self) {
				#expect([UInt8]($0) == expected256)
			}
			try hmac512.finish().RAW_access_immutable(UnsafeRawBufferPointer.self) {
