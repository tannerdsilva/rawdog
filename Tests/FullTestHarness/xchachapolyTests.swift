...
final class xchachapolyTests: XCTestCase {
	func testVectorXChaChaPoly() throws {
		...
		XCTAssertEqual(returnedTag, expectedTag)
		XCTAssertEqual(outputCipher, expectedCiphertext)
	}
    func testXChachaPolyEncryptDecryptRandomData() throws {
		for i in 0..<64 {
			...
			XCTAssertEqual(plaintext, reverseBytes)
		}
    }
}
