...
ge25519_is_neutral_vartime(const ge25519 *p) {
	...
	__crawdog_curve25519_contract(point_buffer[0], p->x);
	__crawdog_curve25519_contract(point_buffer[1], p->y);
	__crawdog_curve25519_contract(point_buffer[2], p->z);
...
int
ED25519_FN(__crawdog_ed25519_sign_open_batch) (const unsigned char **m, size_t *mlen, const unsigned char **pk, const unsigned char **RS, size_t num, int *valid) {
	...
	while (num > 3) {
		...
		ED25519_FN(__crawdog_ed25519_randombytes_unsafe) (batch.r, batchsize * 16);
		...
		for (i = 0; i < batchsize; i++) {
			__crawdog_ed25519_hram(hram, RS[i], pk[i], m[i], mlen[i]);
		...
		if (!ge25519_is_neutral_vartime(&p)) {
			...
			fallback:
			for (i = 0; i < batchsize; i++) {
				valid[i] = ED25519_FN(__crawdog_ed25519_sign_open) (m[i], mlen[i], pk[i], RS[i]) ? 0 : 1;
				ret |= (valid[i] ^ 1);
			}
		}
