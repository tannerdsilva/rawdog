...
DONNA_INLINE static void
__crawdog_curve25519_add_after_basic(bignum25519 out, const bignum25519 a, const bignum25519 b) {
	out[0] = a[0] + b[0];
	out[1] = a[1] + b[1];
	out[2] = a[2] + b[2];
	out[3] = a[3] + b[3];
...
/* out = -a */
DONNA_INLINE static void
__crawdog_curve25519_neg(bignum25519 out, const bignum25519 a) {
	uint64_t c;
...
/* out = a * b */
DONNA_INLINE static void
__crawdog_curve25519_mul(bignum25519 out, const bignum25519 in2, const bignum25519 in) {
...
__crawdog_curve25519_mul_noinline(bignum25519 out, const bignum25519 in2, const bignum25519 in) {
	__crawdog_curve25519_mul(out, in2, in);
}

/* out = in^(2 * count) */
DONNA_NOINLINE static void
__crawdog_curve25519_square_times(bignum25519 out, const bignum25519 in, uint64_t count) {
#if !defined(HAVE_NATIVE_UINT128)
	uint128_t mul;
#endif
	uint128_t t[5];
...
DONNA_INLINE static void
__crawdog_curve25519_square(bignum25519 out, const bignum25519 in) {
#if !defined(HAVE_NATIVE_UINT128)
	uint128_t mul;
#endif
	uint128_t t[5];
...
DONNA_INLINE static void
__crawdog_curve25519_expand(bignum25519 out, const unsigned char *in) {
 ...
 */
DONNA_INLINE static void
__crawdog_curve25519_contract(unsigned char *out, const bignum25519 input) {
	uint64_t t[5];
	uint64_t f, i;
	...
	#define __crawdog_curve25519_contract_carry() \
		t[1] += t[0] >> 51; t[0] &= reduce_mask_51; \
		t[2] += t[1] >> 51; t[1] &= reduce_mask_51; \
		t[3] += t[2] >> 51; t[2] &= reduce_mask_51; \
		t[4] += t[3] >> 51; t[3] &= reduce_mask_51;

	#define __crawdog_curve25519_contract_carry_full() __crawdog_curve25519_contract_carry() \
		t[0] += 19 * (t[4] >> 51); t[4] &= reduce_mask_51;

	#define __crawdog_curve25519_contract_carry_final() __crawdog_curve25519_contract_carry() \
		t[4] &= reduce_mask_51;

	__crawdog_curve25519_contract_carry_full()
	__crawdog_curve25519_contract_carry_full()
	...
	__crawdog_curve25519_contract_carry_final()
...
/* out = (flag) ? in : out */
DONNA_INLINE static void
__crawdog_curve25519_move_conditional_bytes(uint8_t out[96], const uint8_t in[96], uint64_t flag) {
...
/* if (iswap) swap(a, b) */
DONNA_INLINE static void
__crawdog_curve25519_swap_conditional(bignum25519 a, bignum25519 b, uint64_t iswap) {
