...
public struct RAW_staticbuff_access_decl:ExpressionMacro {
	...
	public static func expansion(of node: some SwiftSyntax.FreestandingMacroExpansionSyntax, in context:some SwiftSyntaxMacros.MacroExpansionContext) throws -> SwiftSyntax.ExprSyntax {
		guard node.arguments.count == expectedArgumentCount else {
			let diagnostic = Diagnostic(node:node, message:InternalMacroFailure(message:"expected \(expectedArgumentCount) arguments in RAW_staticbuff_access macro invocation, found \(node.arguments.count)."))
			...
			switch i {
				...
				case 1:
					...
					guard let lastComponent = allNames?.last else {
						let diagnostic = Diagnostic(node:arg, message:InternalMacroFailure(message:"expected a valid key path expression in the second argument of RAW_staticbuff_access, but was not able to find one."))
						context.diagnose(diagnostic)
						return ExprSyntax("()")
					}
				...
				case 4:
					...
					continue argLoop
				default:
					let diagnostic = Diagnostic(node:node, message:InternalMacroFailure(message:"unexpected argument index \(i) in RAW_staticbuff_access macro invocation."))
					context.diagnose(diagnostic)
					return ExprSyntax("()")
			}
		}
		...
		return ExprSyntax(
			"""
			try withUnsafePointer(to:\(raw:storageName.trimmedDescription)) { (ptr:UnsafePointer<Self.RAW_fixed_type>) throws(\(raw:bodyThrowsType.base!)) -> \(raw:bodyReturnType.base!) in
				return try \(raw:bodyArgumentReference)(UnsafeRawBufferPointer(start:ptr, count: MemoryLayout<Self.RAW_fixed_type>.size))
			}
			"""
		)
    }
